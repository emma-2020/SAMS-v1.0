...
// ─── Player Row ──────────────────────────────────────────────────────
function PlayerRow({ player, status, onSet }: { player: RosterPlayer; status: string | null; onSet: (id: string, s: string | null) => void }) {
...
function AttendanceInner() {
  ...
  useEffect(() => {
    ...
    setRosterLoading(true); setRosterError(''); setRosterData(null); setStatuses({});
    (apiClient as any).get(`/attendance?event_id=${selectedEventId}`)
      .then((res: any) => {
        ...
        if (data?.roster) {
          ...
          for (const p of data.roster) { if (p.status) init[p.player_id] = p.status; }
          setStatuses(init);
        }
  ...
  function handleSelectEvent(id: string) {
    ...
    setStatuses({}); setSavedSuccess(false);
  }
          ...
          rosterLoading ? (
            ...
            <>
              ...
              {!roster.length ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {roster.map(p => <PlayerRow key={p.player_id} player={p} status={statuses[p.player_id] ?? null} onSet={handleSetStatus} />)}
                </div>
              )}
