...
function healthStatus(h: CRAPlayer['latest_health']) {
  ...
  if (score >= 70) return { label: 'Good',       color: '#059669', bg: '#ECFDF5', border: '#A7F3D0' };
  if (score >= 40) return { label: 'Moderate',   color: '#6366F1', bg: '#EEF2FF', border: '#C7D2FE' };
  if (score > 0)   return { label: 'Needs Rest', color: '#D97706', bg: '#FFFBEB', border: '#FDE68A' };
  const fatigue = h.fatigue ?? h.energy ?? 0;
  const soreness = h.soreness ?? h.muscle_soreness ?? 0;
  const sleepQ = h.sleep_quality ?? h.sleep ?? 0;
  const avg = (fatigue + soreness + sleepQ) / 3;
...
// ─── Health Ring ────────────────────────────────────────────────────
function HealthRing({ label, value, max = 5, low = false }: { label: string; value: number | null | undefined; max?: number; low?: boolean }) {
  const valid = value != null;
  const pct   = valid ? ((value as number) / max) * 100 : 0;
  const r = 30, sz = 76;
  const circ = 2 * Math.PI * r;
  const dash  = circ - (pct / 100) * circ;
  const isBad  = valid && (low ? (value as number) >= 4 : (value as number) <= 2);
  const isGood = valid && (low ? (value as number) <= 2 : (value as number) >= 4);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
      <div style={{ position: 'relative', width: sz, height: sz }}>
        <svg width={sz} height={sz} style={{ transform: 'rotate(-90deg)', position: 'absolute', inset: 0 }}>
          ...
          {valid && <circle cx={sz/2} cy={sz/2} r={r} fill="none" stroke={stroke} strokeWidth={7} strokeDasharray={circ} strokeDashoffset={dash} strokeLinecap="round" style={{ transition: 'stroke-dashoffset 0.9s cubic-bezier(0.4,0,0.2,1)' }} />}
        </svg>
        <div style={{ position: 'absolute', inset: 7, borderRadius: '50%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          {valid ? (
            <><span style={{ fontSize: '1.05rem', fontWeight: 900, color: stroke, lineHeight: 1 }}>{value}</span><span style={{ fontSize: '0.52rem', fontWeight: 700, color: `${stroke}99` }}>/{max}</span></>
          ) : <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>—</span>}
        </div>
      </div>
      <span style={{ fontSize: '0.67rem', fontWeight: 700, color: 'var(--text-secondary)', textAlign: 'center' }}>{label}</span>
    </div>
  );
}
    ...
    <div onClick={() => onSelect(player)}
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 16px 16px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
          {teams.map(t => (
            <span key={t.id} style={{ fontSize: '0.57rem', fontWeight: 800, padding: '2px 8px', borderRadius: 99, background: 'rgba(99,102,241,0.72)', backdropFilter: 'blur(12px)', color: '#fff', border: '1px solid rgba(99,102,241,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              {t.name}
            </span>
          ))}
        </div>
        <div style={{ fontWeight: 900, fontSize: '1.1rem', color: '#fff', lineHeight: 1.2, letterSpacing: '-0.02em', textShadow: '0 2px 8px rgba(0,0,0,0.4)' }}>
          {player.first_name} {player.last_name}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 3, fontSize: '0.68rem', color: 'rgba(255,255,255,0.52)' }}>
          <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '62%' }}>{player.email}</span>
          <span>{checkinDate ? daysAgo(checkinDate) : 'No check-in'}</span>
        </div>
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,0.13)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgba(255,255,255,0.92)' }}>View Profile</span>
...
// ─── Profile Drawer ─────────────────────────────────────────────────
function ProfileDrawer({ player, onClose }: { player: CRAPlayer; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    coachApi.getPlayerProfile(player.id)
      .then((d: any) => { if (!cancelled) { setProfile(d); setLoading(false); } })
      .catch((e: unknown) => { if (!cancelled) { setError(e instanceof Error ? e.message : 'Failed to load profile.'); setLoading(false); } });
  ...
  const healthLogs    = profile?.healthLogs ?? profile?.health_history ?? [];
  const workouts      = profile?.workouts ?? [];
  const upcomingEvents = profile?.upcomingEvents ?? profile?.schedule ?? [];
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 201, width: 500, maxWidth: '100vw', background: 'var(--bg-surface)', borderLeft: '1px solid var(--border-default)', boxShadow: '0 0 80px rgba(15,23,42,0.2)', display: 'flex', flexDirection: 'column', animation: 'slideInRight 0.28s cubic-bezier(0.34,1.56,0.64,1)' }}>
        ...
        <div style={{ position: 'relative', height: 220, background: '#0F172A', overflow: 'hidden', flexShrink: 0 }}>
          ...
          <div style={{ position: 'absolute', bottom: 18, left: 22, right: 22 }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 7 }}>
              {(player.teams ?? []).map(t => (
                <span key={t.id} style={{ fontSize: '0.58rem', fontWeight: 800, padding: '2px 8px', borderRadius: 99, background: 'rgba(99,102,241,0.78)', backdropFilter: 'blur(8px)', color: '#fff', border: '1px solid rgba(99,102,241,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{t.name}</span>
              ))}
            </div>
            <div style={{ fontWeight: 900, fontSize: '1.5rem', color: '#fff', lineHeight: 1.15, letterSpacing: '-0.025em', textShadow: '0 2px 10px rgba(0,0,0,0.35)' }}>
              {player.first_name} {player.last_name}
            </div>
            <div style={{ fontSize: '0.73rem', color: 'rgba(255,255,255,0.58)', marginTop: 4 }}>{player.email}</div>
          </div>
        </div>

        {/* Status strip */}
        <div style={{ padding: '10px 20px', flexShrink: 0, background: 'var(--bg-elevated)', borderBottom: '1px solid var(--border-subtle)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: '0.72rem', fontWeight: 700, padding: '4px 12px', borderRadius: 99, background: hs.bg, color: hs.color, border: `1px solid ${hs.border}` }}>
            {hs.label === 'Flagged' && <IcoFlag />}
            {hs.label}
            {checkinDate && <span style={{ fontWeight: 500, opacity: 0.75 }}>· {daysAgo(checkinDate)}</span>}
          </span>
        </div>
        ...
        <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
          ...
          {error && <div style={{ padding: 20, textAlign: 'center', color: 'var(--danger)', fontSize: '0.875rem' }}>{error}</div>}
          {!loading && !error && profile && (
            <>
              {tab === 'overview' && <OverviewTab player={player} profile={profile} />}
              {tab === 'health'   && <HealthTab   logs={healthLogs} />}
              {tab === 'workouts' && <WorkoutsTab workouts={workouts} />}
              {tab === 'schedule' && <ScheduleTab events={upcomingEvents} />}
            </>
          )}
        </div>
      </div>
...
function InfoRow({ label, value, valueColor }: { label: string; value?: string; valueColor?: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
      ...
      <span style={{ fontSize: '0.83rem', color: valueColor || 'var(--text-primary)', fontWeight: 600, textAlign: 'right' }}>{value || '—'}</span>
    </div>
  );
}

function OverviewTab({ player, profile }: { player: CRAPlayer; profile: any }) {
  const lh = player.latest_health;
  const checkinDate = lh?.logged_at ?? lh?.submitted_at;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ background: 'var(--bg-elevated)', borderRadius: 16, border: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 11 }}>
          ...
          <InfoRow label="Status"  value={player.is_active !== false ? 'Active' : 'Inactive'} valueColor={player.is_active !== false ? '#059669' : 'var(--text-muted)'} />
          <InfoRow label="Teams"   value={(player.teams ?? []).map(t => t.name).join(', ')} />
          {player.created_at && <InfoRow label="Member Since" value={fmtDate(player.created_at)} />}
        </div>
      </div>

      {lh ? (
        <div style={{ background: 'var(--bg-elevated)', borderRadius: 16, border: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
          <div style={{ padding: '11px 16px', borderBottom: '1px solid var(--border-subtle)', background: 'linear-gradient(90deg, rgba(16,185,129,0.05), transparent)' }}>
            <div style={{ fontWeight: 700, fontSize: '0.84rem', color: 'var(--text-primary)' }}>Latest Health Check-in</div>
            {checkinDate && <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: 2 }}>Logged {daysAgo(checkinDate)}</div>}
          </div>
          <div style={{ padding: '20px 16px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, justifyItems: 'center' }}>
            <HealthRing label="Energy"   value={lh.energy   ?? lh.fatigue}       low={false} />
            <HealthRing label="Sleep"    value={lh.sleep    ?? lh.sleep_quality} low={false} />
            <HealthRing label="Soreness" value={lh.muscle_soreness ?? lh.soreness} low={true} />
          </div>
          {lh.notes && (
            <div style={{ margin: '0 14px 14px', padding: '10px 12px', background: 'rgba(99,102,241,0.05)', borderRadius: 10, border: '1px solid rgba(99,102,241,0.14)', fontSize: '0.76rem', color: 'var(--text-secondary)', fontStyle: 'italic' }}>
              "{lh.notes}"
            </div>
          )}
        </div>
      ) : (
        <div style={{ background: 'var(--bg-elevated)', borderRadius: 16, border: '1px solid var(--border-subtle)', padding: '24px', textAlign: 'center' }}>
...
function HealthTab({ logs }: { logs: any[] }) {
  if (!logs.length) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', textAlign: 'center', gap: 12 }}>
      <div style={{ color: 'var(--text-muted)', fontSize: '1.5rem' }}><IcoHeart /></div>
      <div style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>No health logs</div>
      <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>No wellness check-ins yet.</div>
    </div>
  );
  return (
    <div>
      <div style={{ fontSize: '0.74rem', color: 'var(--text-muted)', marginBottom: 12, fontWeight: 600 }}>Last {logs.length} check-in{logs.length !== 1 ? 's' : ''}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {logs.map((log: any, i: number) => {
          const flagged = log.is_flagged || (log.overall_score ?? 100) < 40;
          const date = log.logged_at ?? log.submitted_at;
          return (
            <div key={log.id ?? i} style={{ background: flagged ? 'linear-gradient(135deg, #FEF2F2 0%, #FFF 60%)' : 'var(--bg-elevated)', border: `1px solid ${flagged ? '#FECACA' : 'var(--border-subtle)'}`, borderRadius: 14, overflow: 'hidden', borderLeft: `4px solid ${flagged ? '#EF4444' : '#10B981'}` }}>
              <div style={{ padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
                <span style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-primary)' }}>{fmtDate(date)}</span>
                {flagged && <span style={{ fontSize: '0.6rem', fontWeight: 800, color: '#EF4444', background: '#FEF2F2', padding: '2px 8px', borderRadius: 99, border: '1px solid #FECACA' }}>FLAGGED</span>}
              </div>
              <div style={{ padding: '14px', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, justifyItems: 'center' }}>
                <HealthRing label="Energy"   value={log.energy   ?? log.fatigue}       low={false} />
                <HealthRing label="Sleep"    value={log.sleep    ?? log.sleep_quality} low={false} />
                <HealthRing label="Soreness" value={log.muscle_soreness ?? log.soreness} low={true}  />
              </div>
              {log.notes && <div style={{ padding: '0 14px 12px', fontSize: '0.73rem', color: 'var(--text-muted)', fontStyle: 'italic' }}>"{log.notes}"</div>}
            </div>
          );
        })}
      </div>
...
function WorkoutsTab({ workouts }: { workouts: any[] }) {
  if (!workouts.length) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', textAlign: 'center', gap: 12 }}>
      <div style={{ color: 'var(--text-muted)', fontSize: '1.5rem' }}><IcoBarbell /></div>
      <div style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>No workouts assigned</div>
      <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Create a training plan from the Workouts page.</div>
    </div>
  );
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {workouts.map((w: any) => {
        ...
        return (
          <div key={w.id} style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 14, padding: '14px 16px', borderLeft: '4px solid #7C3AED' }}>
            <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--text-primary)' }}>{w.title}</div>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginTop: 4 }}>
              {w.due_date ? `Due ${fmtDate(w.due_date)}` : 'No due date'} · {exercises.length} exercises
            </div>
            {exercises.length > 0 && (
              <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 4 }}>
                {exercises.slice(0, 4).map((ex: any, i: number) => (
                  <div key={i} style={{ display: 'flex', gap: 8, fontSize: '0.78rem', color: 'var(--text-secondary)' }}>
                    <span style={{ color: 'var(--text-muted)', flexShrink: 0, minWidth: 18 }}>{i + 1}.</span>
                    <span>{ex.description ?? ex.name}{ex.sets_reps_notes ? ` — ${ex.sets_reps_notes}` : (ex.sets && ex.reps ? ` — ${ex.sets}×${ex.reps}` : '')}</span>
                  </div>
                ))}
              </div>
            )}
...
function ScheduleTab({ events }: { events: any[] }) {
  if (!events.length) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 24px', textAlign: 'center', gap: 12 }}>
      <div style={{ color: 'var(--text-muted)', fontSize: '1.5rem' }}><IcoCal /></div>
      <div style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>No upcoming events</div>
      <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>No sessions scheduled in the near future.</div>
    </div>
  );
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {events.map((ev: any) => (
        <div key={ev.id} style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-subtle)', borderRadius: 14, padding: '12px 14px', display: 'flex', gap: 12, alignItems: 'flex-start', borderLeft: '4px solid #6366F1' }}>
          <div style={{ width: 42, height: 42, borderRadius: 10, flexShrink: 0, background: '#6366F112', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6366F1', border: '1px solid #6366F120' }}>
            <IcoCal />
          </div>
          ...
          <span style={{ fontSize: '0.62rem', fontWeight: 700, padding: '2px 8px', borderRadius: 99, background: '#6366F110', color: '#6366F1', border: '1px solid #6366F128', flexShrink: 0 }}>{ev.type}</span>
        </div>
      ))}
    </div>
  );
...
export default function PlayersPage() {
  ...
  const filtered = players.filter(p => {
    ...
    const tm = activeTeam === 'all' || (p.teams ?? []).some((t: any) => t.id === activeTeam);
    return nm && tm;
  });

  const flaggedCount = filtered.filter(p => p.latest_health?.is_flagged || (p.latest_health?.overall_score ?? 100) < 40).length;
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div className="page-header-row" style={{ marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 'clamp(1.2rem,4vw,1.55rem)', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.025em', lineHeight: 1.2, margin: 0 }}>Player Management</h1>
        ...
        {flaggedCount > 0 && (
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: '0.72rem', fontWeight: 700, padding: '3px 10px', borderRadius: 99, background: '#FEF2F2', color: '#EF4444', border: '1px solid #FECACA' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '1 1 220px', background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 12, padding: '9px 14px', boxShadow: '0 1px 4px rgba(15,23,42,0.04)' }}>
          ...
          {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', padding: 2 }}><IcoClose /></button>}
        </div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '48px 24px', textAlign: 'center', background: 'var(--bg-elevated)', borderRadius: 20, border: '1px solid var(--border-default)' }}>
          <div style={{ fontSize: '2rem', marginBottom: 12 }}>👥</div>
          <div style={{ fontWeight: 600, color: 'var(--text-secondary)' }}>{search ? 'No players match your search' : 'No players yet'}</div>
