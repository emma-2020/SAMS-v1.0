...
async function createInvitation({ adminId, academyId, email, role, firstName, lastName }) {
  ...
  try {
    ...
    // Invitation is still created — admin can resend manually
  }

  return invitation;    // token is NOT in the select — never returned to client
}
...
/**
 * sendInvitationEmail
 * Sends a registration link to the invited user.
 *
 * In production, replace the console.log with nodemailer / Resend / SendGrid.
 * The token is embedded in the frontend registration URL as a query param.
 * The signup endpoint validates it before creating the account.
 */
async function sendInvitationEmail({ to, firstName, role, academyName, token, expiresAt }) {
  const registrationUrl =
    `${process.env.FRONTEND_URL}/register?token=${token}`;

  // --- PRODUCTION: swap the block below for your mail provider ---
  if (process.env.NODE_ENV === 'production') {
    // Example with nodemailer (configure SMTP in .env):
    // const transporter = nodemailer.createTransport({ ... });
    // await transporter.sendMail({ ... });
    // Placeholder until SMTP is wired up:
    console.info(`[EMAIL] Invitation to ${to} → ${registrationUrl}`);
    return;
  }

  // Development: log to console
  console.info('─────────────────────────────────────────');
  console.info('[DEV EMAIL] Invitation Details:');
  console.info(`  To:           ${to}`);
  console.info(`  Name:         ${firstName}`);
  console.info(`  Role:         ${role}`);
  console.info(`  Academy:      ${academyName}`);
  console.info(`  Register URL: ${registrationUrl}`);
  console.info(`  Expires:      ${expiresAt}`);
  console.info('─────────────────────────────────────────');
}
