...
import { uploadAvatar }     from '../../services/auth.api';
...
// ─── Form input with managed focus ring ──────────────────────────
...
// ─── Form field wrapper with label + description ──────────────────
function FormField({ label, description, children }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{
        fontSize: '0.77rem', fontWeight: 700, color: 'var(--text-secondary)',
    ...
    <button
      ...
      onMouseEnter={e => {
        if (!disabled && !loading) {
          ...
          e.currentTarget.style.boxShadow = '0 8px 20px rgba(99,102,241,0.44)';
        }
      }}
      onMouseLeave={e => {
        e.currentTarget.style.transform = 'translateY(0)';
        e.currentTarget.style.boxShadow = '0 4px 12px rgba(99,102,241,0.32)';
      }}
    >
      {loading
    ...
    <div style={{
      ...
      {copyable && (
        <button onClick={onCopy} style={{
          ...
          background: copied ? 'var(--success-subtle)' : 'var(--bg-surface)',
          border: `1.5px solid ${copied ? 'var(--success-border)' : 'var(--border-default)'}`,
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div>
          <div style={{ fontSize: '0.875rem', fontWeight: 700, color: active ? 'var(--accent-dim)' : 'var(--text-primary)' }}>
            {label}
          </div>
...
export default function SettingsPage() {
  ...
  function handleAvatarPick(e) { handleFileSelect(e.target.files?.[0]); }
  ...
  async function handleAvatarUpload() {
    ...
    try {
      const profile = await uploadAvatar(avatarFile);
  ...
  async function handleProfileSave(e) {
    e.preventDefault(); setProfileSuccess(false);
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease', maxWidth: 780 }}>
      ...
      <div style={{
        ...
        border: `1.5px solid ${meta.hex}18`,
        borderRadius: 20,
        marginBottom: 28,
        ...
        <div style={{
          ...
          background: `linear-gradient(135deg, ${meta.hex}, ${meta.hex}60)`,
          boxShadow: `0 0 0 5px ${meta.hex}14, 0 4px 16px ${meta.hex}22`,
        }}>
          <div style={{
            ...
            {user?.avatar_url ? (
              ...
              <span style={{ fontWeight: 900, fontSize: '1.55rem', color: meta.hex, letterSpacing: '-0.03em' }}>
                {initials}
              </span>
            )}
          </div>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{
            fontWeight: 800, fontSize: '1.45rem', color: 'var(--text-primary)',
          ...
          <div style={{
            ...
            background: `${meta.hex}12`, border: `1px solid ${meta.hex}28`,
          }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: meta.hex, flexShrink: 0 }} />
            <span style={{
              fontWeight: 700, fontSize: '0.7rem',
              letterSpacing: '0.1em', textTransform: 'uppercase', color: meta.hex,
            }}>
              {meta.label}
            </span>
          </div>
      ...
      {activeTab === 'profile' && (
        <div style={{ animation: 'fadeIn 0.2s ease', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {/* Photo upload card */}
          <SettingsCard
            icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>}
            title="Profile Photo"
            subtitle="Shown across the platform"
          >
            <input
              ref={avatarInputRef}
              type="file"
                ...
                <>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: '#94A3B8', marginTop: 3 }}>
                      Drop a new photo or{' '}
                      <span style={{ color: '#6366F1', fontWeight: 600 }}>click to change</span>
                    </div>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, color: '#334155', fontSize: '0.9rem' }}>
                      ...
                      <span style={{ color: '#6366F1', fontWeight: 700, textDecoration: 'underline', textUnderlineOffset: 2 }}>
                        browse
                      </span>
                    </div>
            ...
            {(avatarFile || user?.avatar_url) && (
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 16 }}>
                ...
                {avatarFile && (
                  <SaveButton loading={avatarUploading} type="button" onClick={handleAvatarUpload}>
                    Upload Photo
                  </SaveButton>
                )}
              </div>
            ...
            {avatarError && (
              <div className="alert alert-error" style={{ padding: '9px 14px', marginTop: 12, fontSize: '0.82rem' }}>
                {avatarError}
              </div>
            )}
            {avatarSuccess && (
              <div className="alert alert-success" style={{ padding: '9px 14px', marginTop: 12, fontSize: '0.82rem' }}>
                <IcoCheck /> Profile photo updated!
              </div>
            )}
          </SettingsCard>
      ...
      {activeTab === 'security' && (
        <div style={{ animation: 'fadeIn 0.2s ease', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <SettingsCard
            ...
            subtitle="Your current login session"
          >
            <div style={{
              display: 'flex', alignItems: 'center', gap: 14,
              padding: '14px 16px', background: 'var(--bg-elevated)',
              borderRadius: 12, border: '1.5px solid var(--border-subtle)',
            }}>
              <div style={{
                ...
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
              </div>
              <div style={{ flex: 1 }}>
              ...
              <span style={{
                ...
                color: 'var(--success)', letterSpacing: '0.06em', textTransform: 'uppercase',
              }}>
                Active
              </span>
            </div>
        ...
        <div style={{ animation: 'fadeIn 0.2s ease', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <SettingsCard icon={<IcoPalette />} title="Theme" subtitle="Choose your preferred colour scheme">
            <div style={{ display: 'flex', gap: 16 }}>
              <ThemeCard
                value="light" label="Light Mode" description="Clean and bright" current={theme}
                onClick={() => theme === 'dark' && toggleTheme()}
                preview={{ bg:'#F4F6FA', border:'#E2E8F0', bar:'#0F172A', line:'#94A3B8', card:'#FFFFFF' }}
              />
              <ThemeCard
                value="dark" label="Dark Mode" description="Easy on the eyes" current={theme}
          ...
          <SettingsCard
            ...
            subtitle="Adjust content spacing"
          >
            {[
              ...
              return (
                <label key={opt.id} onClick={() => setDensity(opt.id)} style={{
                  display: 'flex', alignItems: 'center', gap: 14,
                  padding: '14px 16px',
                  background: isActive ? 'var(--accent-subtle)' : 'var(--bg-elevated)',
                  border: `1.5px solid ${isActive ? 'var(--accent-light)' : 'var(--border-subtle)'}`,
                  borderRadius: 12, cursor: 'pointer', marginBottom: i < 2 ? 10 : 0,
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.875rem', fontWeight: 700, color: isActive ? 'var(--accent-dim)' : 'var(--text-primary)' }}>
                      {opt.label}
                    </div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>
                      {opt.desc}
                  ...
                  {isActive && (
                    <span style={{
                      ...
                      color: 'var(--accent-dim)', padding: '3px 9px', borderRadius: 99,
                      background: 'var(--accent-subtle)', border: '1px solid var(--accent-light)',
                    }}>
                      Active
                    </span>
        ...
        <div style={{ animation: 'fadeIn 0.2s ease', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <SettingsCard icon={<IcoBldg />} title="Academy Information" subtitle="Your academy connection details">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <InfoRow label="Member Since"   value={user?.created_at ? new Date(user.created_at).toLocaleDateString('en-GB', { day:'numeric', month:'long', year:'numeric' }) : '—'} />
            </div>
          </SettingsCard>
