...
export async function getMessages(channelId: string): Promise<ChatMessage[]> {
  const res = (await apiClient.get(`/chat?channel_id=${channelId}&limit=50`)) as {
    success: boolean;
    data: { messages: Parameters<typeof mapChatMessage>[0][] };
  };
  return (res.data?.messages ?? []).map(mapChatMessage);
}

// Backend: POST /chat  { channel_id, message_text?, attachment_url?, client_message_id?, ... }
// clientMessageId should be generated by the caller (e.g. crypto.randomUUID())
// and reused for the optimistic-UI message id — that way, if this send is
// queued offline and retried later, the server can de-dupe the retry, and
// the caller can match the eventual queue "sent" event back to its optimistic
// bubble using the same id.
export async function sendMessage(
  ...
  const res = (await apiClient.post('/chat', payload)) as {
    success: boolean;
    data: { message: Parameters<typeof mapChatMessage>[0] };
  };
